...
```{r}
quad_f = function(x1,x2) {quad_fit$coefficients[1] + quad_fit$coefficients[2] * x1 + quad_fit$coefficients[2] * x2 + quad_fit$coefficients[3] * x1 * x2 + quad_fit$coefficients[4] * x1^2 + quad_fit$coefficients[2] * x2^2 - 0.5}

X1 = seq(-2.5, 3, length = 100)
X2 = seq(-1.5, 4, length = 100)
Z = outer(X1, X2, quad_f)

#ggplot(data = train_df, aes(x = x1, y = x2, color = as.factor(y))) + 
#    geom_point() + geom_contour(aes(x = X1, y = X2, z = Z), color = "grey32", #inherit.aes = FALSE)
