...
simpleLinearRegression = function(index) {
  ...
  linearTrainErr[index,"errorRate"] = mean(train_df$y != lm_pred_cl_train)
  linearTestErr[index,"errorRate"] = round(mean(ifelse(predict(lm_fit, newdata = test_df, interval = "prediction") > 0.5, 1, 0) != test_df$y),4)
}
...
quadraticRegression = function(index) {
  ...
  quadTrainErr[index,"errorRate"] = mean(train_df$y != lm_pred_cl_train)
  quadTestErr[index,"errorRate"] = round(mean(ifelse(predict(quad_fit, newdata = test_df, interval = "prediction") > 0.5, 1, 0) != test_df$y),4)
}
```
...
cvKNN = function(dataSet, foldNum) {
    ...
    result$train.cvError = cvKNNAveErrorRates[bestK_index]

    predictY = knn(train_df, test_df, cl_train, k = result$bestK)
    error = sum(predictY != cl_test) / nrow(test_df)
    result$cvError = error
    result
}
...
kNearestNeighbor = function(index) {
  ...
  knnTestErr[index,"errorRate"] = result$cvError
  knnTrainErr[index,"errorRate"] = result$train.cvError
  KNNCvBestK[index] = (result$bestK)
}
...
bayesError = function(index) {
  cl_train_pred_Bayes = apply(train_data, 1, mixedModelBayes)
  cl_train_pred_Bayes = as.numeric(cl_train_pred_Bayes > 1);
  train.err.Bayes = sum(cl_train !=  cl_train_pred_Bayes) / (2*nrow(train_data))
  bayesTrainErr[index,"errorRate"] = train.err.Bayes
  ...
  bayesTestErr[index,"errorRate"] = test.err.Bayes
}
```
...
simSize = 20;
for(i in 1:simSize) {
  simpleLinearRegression(i)
  quadraticRegression(i)
  kNearestNeighbor(i)
  bayesError(i)
}
