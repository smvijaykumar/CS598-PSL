...
simpleLinearRegression = function(index) {
  ...
  test_error = round(mean(ifelse(predict(lm_fit, newdata = test_df, interval = "prediction") > 0.5, 1, 0) != test_df$y),4))
}
```
...
quadraticRegression = function(index) {
  ...
  c(train_error = mean(train_df$y != lm_pred_cl_train),
  test_error  = round(mean(ifelse(predict(quad_fit, newdata = test_df, interval = "prediction") > 0.5, 1, 0) != test_df$y),4))
}
```
...
cvKNN = function(dataSet, foldNum) {
    ...
    predictY = knn(train_df, test_df, train_df$y, k = result$bestK)
...
```{r}
results = rbind(c(min(linearTrainErr$errorRate), min(linearTestErr$errorRate)),
c(min(quadTrainErr$errorRate), min(quadTestErr$errorRate)),
c(min(knnTrainErr$errorRate), min(knnTestErr$errorRate)),
c(min(bayesTrainErr$errorRate), min(bayesTestErr$errorRate)))
colnames(results) = c("Train Error","Test Error")
rownames(results) = c("Linear Regression","Quadratic L.Regression","KNN","Bayes Rule")
results
```

- The above table shows the training and test errors for four procedures which we analyzed in this study. `KNN` has low test error among the models. 
