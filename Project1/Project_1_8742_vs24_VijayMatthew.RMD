...
- 10 Fold Cross Validation to choose Lamda hyperparameter
...
- Feature Engineering to select variables with non zero coefficients
...
tst.err=RMSE(test.y$Sale_Price_Log,preds)
...
xgbFit = xgboost(data = as.matrix(trainData[,!names(trainData) %in% c("Sale_Price","Sale_Price_Log","PID")]), label = as.matrix(trainData$Sale_Price_Log), 
    nrounds = best_xg_param$iter, verbose = FALSE, objective = "reg:squarederror", eval_metric = "rmse", 
    eta = best_xg_param$eta, max_depth = best_xg_param$max_depth, subsample = best_xg_param$subsample)

## Predictions
# rmse of training data
predict_xg_train = predict(xgbFit, newdata = as.matrix(trainData[,!names(trainData) %in% c("Sale_Price","Sale_Price_Log","PID")]))
tr.err = RMSE(trainData$Sale_Price_Log, predict_xg_train)
# rmse of testing data
preds2 <- predict(xgbFit, newdata = as.matrix(testData[,!names(testData) %in% c("PID")]))
tst.err = RMSE(test.y$Sale_Price_Log, preds2)
