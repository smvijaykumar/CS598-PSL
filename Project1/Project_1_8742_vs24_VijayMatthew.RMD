...
```{r eval=FALSE}
  # Find zero varience factors using caret and remove
...
for(j in 1:n) {
  ...
  preds_train<-predict(lasso_mod,newx=as.matrix(X),s=cv_lasso$lambda.min, alpha = 1)
  lasso.train.error[j] = (RMSE(trainData$Sale_Price_Log,preds_train))

  missing = setdiff(train_lasso_vars,names(testData))
  testData[missing] = 0
  preds_test<-predict(lasso_mod,newx=as.matrix(testData[,train_lasso_vars]),s=cv_lasso$lambda.min, alpha = 1)
