...
Since one full run takes about 1 hour on a MSI Intel i7 16G memory laptop on the full dataset of ratings, we speed up the processing of this markdown document by running a reduced dataset (10% of full dataset) evaluation only once in this file.

We added another markdown file: Parallel.RMD which evaluates each model 10 iterations on full dataset using parallel and foreach package to speed up the process. 

From the following table of prediction speed and lowest RMSE on both reduced dataset and full dataset, System I Average Popular model stands out.
...
kable(data.frame(SystemI_Avg_Popular="334.793 secs (~6min)", SystemI_Hybrid="13856.588 secs (~4hrs)", SystemII_UBCF="6924.217 secs (~2hrs)", SystemII_IBCF="798.802 secs (~13min)"))
