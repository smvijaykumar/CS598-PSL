...
for (u in 1:nrow(test)){
    ...
    rating = POPULAR_rec_list[[userid]][movieid]
    # handle missing values; 2.5 might not be the ideal choice
    POPULAR_test.pred$rating[u] = ifelse(is.na(rating), 2.5, rating)
}
...
### System Ib - Genre based (Highly rated)


- Train a genre based recommender system by popularity using R package [recommenderlab]

```{r}
rec_RERECOMMEND = Recommender(Rmat, method = 'RERECOMMEND',
                          parameter = list(randomize = 1, minRating = 1))

#Summary of model parameters
rec_RERECOMMEND@model
```
...
```{r}
# predict ratings for the first three users
RERECOMMEND_recom = predict(rec_RERECOMMEND, 
                Rmat[1:3], type = 'ratings')
```
...
```{r}
as(RERECOMMEND_recom, 'matrix')[, 1:10]
as(Rmat, 'matrix')[1:3, 1:10]
```
...
RERECOMMEND_recom = predict(rec_RERECOMMEND, 
                Rmat, type = 'ratings')  
RERECOMMEND_rec_list = as(RERECOMMEND_recom, 'list')  # each element are ratings of that user

RERECOMMEND_test.pred = test
RERECOMMEND_test.pred$rating = NA

RERECOMMEND_time = Sys.time() - start.time
```
...
for (u in 1:nrow(test)){
    ...
    rating = RERECOMMEND_rec_list[[userid]][movieid]
    # handle missing values; 2.5 might not be the ideal choice
    RERECOMMEND_test.pred$rating[u] = ifelse(is.na(rating), 2.5, rating)
}

# Calculate RMSE
RERECOMMEND_RMSE = sqrt(mean((test$Rating - RERECOMMEND_test.pred$rating)^2)) 
RERECOMMEND_RMSE
```
...
- Train a user-based collaborative filtering recommender system using R package [recommenderlab]
...
for (u in 1:nrow(test)){
    ...
    # handle missing values; 2.5 might not be the ideal choice
    UBCF_test.pred$rating[u] = ifelse(is.na(rating), 2.5, rating)
}
...
- Train Item-based collaborative filtering recommender system using R package recommenderLab
...
for (u in 1:nrow(test)){
    ...
    # handle missing values; 2.5 might not be the ideal choice
    IBCF_test.pred$rating[u] = ifelse(is.na(rating), 2.5, rating)
}
...
```{r echo=FALSE}
results = data.frame("System I: Popular" = POPULAR_RMSE, "System I: Highly Rated" = RERECOMMEND_RMSE, "System II: UBCF"= UBCF_RMSE, "System II: IBCF"= IBCF_RMSE)
...
results2 = data.frame("System I: Popular" = POPULAR_time, "System I: Highly Rated" = RERECOMMEND_time, "System II: UBCF"= UBCF_time, "System II: IBCF"= IBCF_time)
